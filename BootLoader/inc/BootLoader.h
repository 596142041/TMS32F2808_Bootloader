...
#define APP_INFO_ADDR        ((uint32_t)0x3E8000)
#define APP_START_ADDR       ((uint32_t)0x3E8010)
#define APP_Write_END_ADDR   ((uint32_t)0x3F4000)
#define APP_Write_START_ADDR ((uint32_t)0x3E8000)
...
//---------------------------------------------------------------------------------
//故障信息列表
#define DEVICE_ADDR_ERROR  0xA0 //设备地址错误
#define ERASE_ERROR        0xA1 //擦除错误
#define WRITE_ERROR        0xA2 //写入数据错误
#define READ_LEN_ERROR     0xA3 //读取数据长度错误
#define MSG_DATA_LEN_ERROR 0xA4 //消息长度错误
#define FILE_TYPE_ERROR    0xA5 //烧写文件类型错误
#define CRC_ERROR          0xA6 //CRC校验错误
#define FLASH_ADDR_ERROR   0xA7 //FALSH地址错误
#define WRITE_LEN_ERROR    0xA8 //写入数据长度错误
//---------------------------------------------------
//-----------烧写文件类型宏定义----------------------
/****************************************************
 * 当前DSP芯片和STM32系列芯片支持bin文件和hex文件烧写,但是STM32芯片仅仅支持keil生成的hex文件,IAR测试有问题具体无法确认
 ****************************************************/
#define File_None 0xF0 //烧写文件类型未知
#define File_bin  0xF1 //烧写文件为bin文件
#define File_hex  0xF2 //烧写文件为hex文件,
...
typedef struct _Device_INFO
{
    ...
    unsigned short int Sector_size;//FLASH扇区大小,仅仅针对DSP芯片有效,
} Device_INFO;
